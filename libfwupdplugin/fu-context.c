...
/**
 * fu_context_get_bios_setting_pending_value:
 * @self: a #FuContext
 * @id: a BIOS setting title
 * @value: a BIOS setting value
 * @error: (nullable): optional return location for an error
 *
 * Set a new pending BIOS setting to be wrote.
 *
 * Returns: %TRUE if successfully updated a pending value.
 *
 * Since: 1.9.4
 **/
gboolean
fu_context_get_bios_setting_pending_value(FuContext *self,
					  const gchar *id,
					  const gchar *value,
					  GError **error)
{
