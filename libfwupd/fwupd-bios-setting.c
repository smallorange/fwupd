...
/**
 * fwupd_bios_setting_get_pending_value:
 * @self: a #FwupdBiosSetting
 *
 * Get a pending BIOS pending value from the cache. This value is not
 * applied to the BIOS.
 *
 ...
 **/
const gchar *
fwupd_bios_setting_get_pending_value(FwupdBiosSetting *self)
{
	FwupdBiosSettingPrivate *priv = GET_PRIVATE(self);
	g_return_val_if_fail(FWUPD_IS_BIOS_SETTING(self), NULL);
	return priv->pending_value;
}

/**
 * fwupd_bios_setting_set_pending_value:
 * @self: a #FwupdBiosSetting
 ...
 **/
void
fwupd_bios_setting_set_pending_value(FwupdBiosSetting *self, const gchar *value)
{
	FwupdBiosSettingPrivate *priv = GET_PRIVATE(self);

	/* not changed */
	if (g_strcmp0(priv->pending_value, value) == 0)
		return;
